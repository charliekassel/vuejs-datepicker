...
const utils = {
  ...
  /**
   * Returns the week of year, using UTC or not
   * @param {Date} date
   * @return {Number}
   */
  getWeek (date) {
    ...
    tempDate.setDate(this.getDate(tempDate) + 3 - (this.getDay(tempDate) + 6) % 7)
    // January 4 is always in week 1.
    const week1 = new Date(this.getFullYear(tempDate), 0, 4)
    // Adjust to Thursday in week 1 and count number of weeks from date to week1.
    return 1 + Math.round(((tempDate.getTime() - week1.getTime()) / 86400000 - 3 + (this.getDay(week1) + 6) % 7) / 7)
  },
